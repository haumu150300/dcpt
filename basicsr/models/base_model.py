...
class BaseModel:
    ...
    def validation(
        ...
        """
        if self.opt["dist"]:
            self.dist_validation(dataloader, current_iter, tb_logger, save_img, clamp)
        else:
    ...
    def model_to_device(self, net, dist=True, find_unused_parameters=None):
        ...
        elif self.opt["num_gpu"] > 1 and dist:
    ...
    def get_optimizer(self, optim_type, params, lr, **kwargs):
        ...
        #     optimizer = apex.optimizers.FusedAdam(params, lr, **kwargs)
        else:
    ...
    @master_only
    def print_network(self, net, shape=(1, 3, 64, 64)):
        ...
        demo_path = "datasets/classical/LR/LRBI/Set5/x4/baby.png"
        if not os.path.exists(demo_path):
            demo_path = "datasets/classical/LR/LRBI/Set5/x4/baby_LRBI_x4.png"

        net = self.get_bare_model(net)
        try:
            img = (
                cv2.cvtColor(
                    cv2.imread(demo_path),
                    cv2.COLOR_BGR2RGB,
                )
                / 255.0
            )
            data = torch.from_numpy(img).permute(2, 0, 1).unsqueeze(0).cuda().float()
            model_stats = summary(net, verbose=0, input_data=data)
        except:
        ...
        logger.info(f"The shape of feed-{net_cls_str}-data is {shape}.")
    ...
    def load_network(
        self, net, load_path, strict=True, param_key="params", remove_norm=False
    ):
        """Load network.
